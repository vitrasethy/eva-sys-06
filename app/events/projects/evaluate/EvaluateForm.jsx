    ...
    <form className="space-y-6" action={actionWithProp}>
      ...
      {data.category.map((cat, id) => (
        <div key={cat.id}>
          ...
          {cat.criteria.map((cri, index) => (
            <div
              key={cri.id}
              className="md:flex md:justify-around my-10 p-4 sm:px-6 rounded-lg bg-[#f7f9f9] border-2 border-gray-300 "
            >
              <div className="md:w-1/2">
                <h2 className="text-gray-900 flex text-lg rounded-lg peer-checked:ring-blue-500 peer-checked:border-blue-500 w-full p-2.5">
